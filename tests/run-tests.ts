...
await import("./driver-account-surfaces-localization.test.ts");
...
// Canonical behavioral coverage replaces the retired legacy dashboard, Stripe-status,
// billing, and admin-payment fixtures. See focused driver/payment/onboarding suites for
// current identity, canonical obligation, batch, webhook, cutoff, and provider-safety rules.
...
test("owner add location returns missing profile fields when blocked", async () => {
  ...
  await withPatchedStorage(
    ...
    async () => {
      ...
      assert.match(body.message || "", /complete your facility profile/i);
    },
  );
});
...
// Canonical rewards and lottery coverage now lives in tests/canonical-rewards-lottery.test.ts.
// It covers period lifecycle; ticket eligibility and idempotency; ineligibility reasons;
// draw safeguards; winner relationships; cancellation announcements; and isolation from
// billing and provider execution.
