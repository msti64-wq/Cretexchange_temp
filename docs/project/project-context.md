...
[CTX-ARCH-017](../architecture/CTX-ARCH-017-two-factor-authentication-architecture.md), proposed [PD-063](../product/PD-063-two-factor-authentication-and-account-recovery-policy.md), [CTX-UX-010](../ux/CTX-UX-010-two-factor-authentication-experience.md), draft [CTX-RB-011](../operations/CTX-RB-011-two-factor-authentication-recovery-and-reset-runbook.md), and the [Phase 5 Sprint 3 plan](./phase-5-sprint-3-two-factor-authentication-plan.md) record the discovery checkpoint. They are drafts and do not authorize implementation.

The active authentication path is username/password followed by a seven-day bearer JWT stored in browser local storage. The server checks user activity and `auth_token_version`, but no MFA factor, server-side device/session ledger, authentication rate limiter, external email/SMS delivery adapter, or append-only authentication-security event domain exists. Password change/reset does not currently revoke all existing JWTs. The repository contains an OTP input component but no TOTP, OTP, WebAuthn/passkey, or recovery-code implementation.
