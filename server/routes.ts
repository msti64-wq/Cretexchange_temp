...
import {
  ...
  type DriverStripeOnboardingResolution,
  type DriverStripeReconciliationField,
} from "./driverStripeService";
...
const DRIVER_STRIPE_ACCOUNT_LINK_CONFIG_ADMIN_MESSAGE =
  'Set PUBLIC_APP_URL, APP_BASE_URL, or RAILWAY_PUBLIC_DOMAIN to the public HTTPS app URL before enabling driver Stripe payouts.';
...
function normalizeConfiguredAppUrl(value: string, source: string) {
...
function getConfiguredPublicAppUrl() {
  ...
  if (publicAppUrl) {
    return { value: normalizeConfiguredAppUrl(publicAppUrl, 'PUBLIC_APP_URL'), source: 'PUBLIC_APP_URL' };
  }

  const appBaseUrl = process.env.APP_BASE_URL?.trim();
  if (appBaseUrl) {
    return { value: normalizeConfiguredAppUrl(appBaseUrl, 'APP_BASE_URL'), source: 'APP_BASE_URL' };
  }

  const railwayPublicDomain = process.env.RAILWAY_PUBLIC_DOMAIN?.trim();
  if (railwayPublicDomain) {
    return {
      value: normalizeConfiguredAppUrl(railwayPublicDomain, 'RAILWAY_PUBLIC_DOMAIN'),
...
function logDriverStripeAccountLinkConfig(
  ...
  console.info('[DRIVER_STRIPE_ACCOUNT_LINK]', {
    maskedAccountId: maskStripeAccountId(stripeConnectAccountId),
...
const DRIVER_STRIPE_ACCOUNT_LINK_AFTER_ACCOUNT_CREATED_MESSAGE =
...
function getDriverStripeAccountLinkFailureMessage(params: {
  hasStripeAccount: boolean;
  statusCode: number;
  isConfigError: boolean;
  stripeError?: any;
}) {
  if (params.hasStripeAccount) {
    return DRIVER_STRIPE_ACCOUNT_LINK_AFTER_ACCOUNT_CREATED_MESSAGE;
  }

  if (params.isConfigError) {
    return DRIVER_STRIPE_PLATFORM_SETUP_UNAVAILABLE_MESSAGE;
  }

  if (params.statusCode === 400 && params.stripeError?.message) {
    return params.stripeError.message;
  }

  return 'Failed to create Stripe onboarding link. Please try again.';
}

type DriverStripeOnboardingStatus =
  | 'not_started'
  | 'setup_started'
  | 'action_required'
  | 'payouts_ready';

function getDriverStripeOnboardingStatusLabel(status: DriverStripeOnboardingStatus) {
  switch (status) {
    case 'payouts_ready':
...
function getDriverStripeOnboardingStatus(params: {
  hasAccount: boolean;
  detailsSubmitted: boolean;
  payoutsEnabled: boolean;
  requirementsCurrentlyDue: string[];
  requirementsPastDue: string[];
}): DriverStripeOnboardingStatus {
...
function buildDriverStripeStatusResponse(
  stripeConnectAccountId: string | null | undefined,
  account?: Stripe.Account | null,
) {
  const accountId = account?.id || stripeConnectAccountId || null;
  ...
  return {
    ...
    accountId,
    status,
    statusLabel: getDriverStripeOnboardingStatusLabel(status),
    detailsSubmitted,
    ...
    hasBlockingRequirements: requirementsCurrentlyDue.length > 0 || requirementsPastDue.length > 0,
  };
}
...
async function getStripeConnectSetupHealth() {
  ...
  try {
    ...
    return {
      ...
      message: 'Stripe Connect API is reachable. Driver payout onboarding requests the Express connected-account transfers capability for ACH payouts.',
...
export async function createDriverStripePayoutAccount(
  ...
  if (!validation.isComplete) {
    const error = new Error('Complete required Stripe payout profile fields before setting up Stripe payouts.') as Error & {
      statusCode?: number;
      missingFields?: string[];
      invalidFields?: string[];
    };
  ...
  if (typeof stripe.accounts.list === 'function') {
    const existingAccount = await stripeService.findConnectedAccountByUserId(user.id);
    if (existingAccount) {
      console.log('[DRIVER_PAYOUT_SETUP] Reusing Stripe Connect account found by user metadata:', {
        userId: user.id,
        driverId: driver?.id || null,
        maskedAccountId: maskStripeAccountId(existingAccount.id),
      });
  ...
  console.log('[STRIPE_ACCOUNT_CREATE_PAYLOAD]', {
    ...
    userId: user.id,
    driverId: driver?.id || null,
    accountType: accountParams.type,
    country: accountParams.country,
    hasEmail: Boolean(accountParams.email),
    requestedCapabilities: Object.keys(accountParams.capabilities || {}),
  });
...
export async function createDriverStripeOnboardingLink(
  ...
  try {
    ...
    if (account) {
      console.info('[driver.stripe.status_checked]', {
        source: 'createDriverStripeOnboardingLink',
    ...
    console.warn('[driver.stripe.status_unavailable]', {
      source: 'createDriverStripeOnboardingLink',
      maskedAccountId: maskStripeAccountId(stripeConnectAccountId),
...
function toDriverStripeAccountSnapshot(account: Stripe.Account | Stripe.DeletedAccount): DriverStripeAccountSnapshot {
  if ('deleted' in account && account.deleted) {
    return { id: account.id, deleted: true };
  }

  const activeAccount = account as Stripe.Account;
  const externalAccounts = activeAccount.external_accounts as any;
  return {
    id: activeAccount.id,
    email: activeAccount.email,
    metadata: activeAccount.metadata,
    details_submitted: activeAccount.details_submitted,
    payouts_enabled: activeAccount.payouts_enabled,
    charges_enabled: activeAccount.charges_enabled,
    capabilities: {
      transfers: activeAccount.capabilities?.transfers || null,
    },
    requirements: {
      currently_due: activeAccount.requirements?.currently_due || [],
      past_due: activeAccount.requirements?.past_due || [],
    },
    external_accounts: {
      data: Array.isArray(externalAccounts?.data)
        ? externalAccounts.data.map((externalAccount: any) => ({
            object: externalAccount?.object,
            type: externalAccount?.type,
          }))
        : [],
      total_count: typeof externalAccounts?.total_count === 'number'
        ? externalAccounts.total_count
        : undefined,
    },
  };
}

async function findDriverStripeAccountsByIdentity(params: {
  userId: string;
  driverId: string | null;
  email: string | null;
}): Promise<DriverStripeAccountSnapshot[]> {
  const matches: DriverStripeAccountSnapshot[] = [];
  ...
  while (hasMore) {
    ...
    for (const account of page.data) {
      ...
      const metadataDriverId = metadata.driverId || metadata.driver_id;
      const accountEmail = account.email?.trim().toLowerCase() || null;
      const matchesIdentity = metadataUserId === params.userId
        || Boolean(params.driverId && metadataDriverId === params.driverId)
        || Boolean(normalizedEmail && accountEmail === normalizedEmail);
      if (matchesIdentity) {
        matches.push(toDriverStripeAccountSnapshot(account));
      }
    }

    hasMore = page.has_more;
    startingAfter = hasMore && page.data.length > 0
      ? page.data[page.data.length - 1].id
      : undefined;
    if (hasMore && !startingAfter) {
      throw new Error('Stripe account discovery pagination could not advance');
    }
  }
...
const canonicalDriverStripeService = createDriverStripeService({
  retrieveAccount: async (accountId) => toDriverStripeAccountSnapshot(await stripe.accounts.retrieve(accountId)),
  findAccountsByIdentity: findDriverStripeAccountsByIdentity,
  reconcileAccountIds: (params) => storage.reconcileDriverStripeAccountIds(params),
  auditLogger: (event, details) => console.info(`[${event}]`, details),
});

const driverStripeOnboardingLocks = new Map<string, Promise<void>>();

async function withDriverStripeOnboardingLock<T>(userId: string, action: () => Promise<T>): Promise<T> {
  const previous = driverStripeOnboardingLocks.get(userId) || Promise.resolve();
  let releaseCurrent: () => void = () => undefined;
  const current = new Promise<void>((resolve) => {
    releaseCurrent = resolve;
  });
  const queued = previous.then(() => current);
  driverStripeOnboardingLocks.set(userId, queued);
  ...
  try {
    ...
    releaseCurrent();
    if (driverStripeOnboardingLocks.get(userId) === queued) {
      driverStripeOnboardingLocks.delete(userId);
    }
  }
}

type DriverStripeSafeAccountResult = {
  ok: boolean;
...
async function safelyResolveDriverStripeOnboardingAccount(params: {
  user: User;
  driver: Driver;
  source: string;
}): Promise<DriverStripeSafeAccountResult> {
  const resolve = (user: User, driver: Driver) => canonicalDriverStripeService.resolveDriverStripeAccountForOnboarding({
    user,
    driver,
  ...
  const decision = await coordinateDriverStripeOnboarding({
    ...
    beforeFinalResolution: async () => {
      const profileValidation = await validateDriverStripePayoutProfile(params.user);
      if (!profileValidation.isComplete) {
        ...
        error.invalidFields = profileValidation.invalidFields;
        throw error;
      }
    },
    reload: async () => {
      const [freshUser, freshDriver] = await Promise.all([
        storage.getUser(params.user.id),
        storage.getDriver(params.user.id),
      ]);
      return freshUser && freshDriver ? { user: freshUser, driver: freshDriver } : null;
    },
  });

  if (decision.action === 'blocked' || (!decision.resolution.accountId && decision.action === 'reuse')) {
    console.warn('[driver.stripe.creation_blocked]', {
      userId: decision.user.id,
      driverId: decision.driver.id,
      source: params.source,
      canonicalStatus: decision.resolution.status,
      discoveryOutcome: decision.resolution.discoveryOutcome,
      errorCode: decision.resolution.errorState?.code || null,
    });
    return {
      ok: false,
      accountId: null,
  ...
  if (decision.action === 'reuse') {
    const reconciliation = await canonicalDriverStripeService.executeDriverStripeReconciliation({
      user: decision.user,
    ...
    console.info('[driver.stripe.account_reused]', {
      userId: decision.user.id,
      driverId: decision.driver.id,
      source: params.source,
      canonicalStatus: decision.resolution.status,
      discoveryOutcome: decision.resolution.discoveryOutcome,
      maskedAccountId: maskStripeAccountId(decision.resolution.accountId),
      fieldsReconciled: reconciliation.updatedFields,
    });
    return {
      ok: true,
      accountId: decision.resolution.accountId,
      created: false,
      reused: true,
      updatedFields: reconciliation.updatedFields,
      resolution: decision.resolution,
    };
  }
  ...
  if (reconciliation.conflict) {
    console.error('[driver.stripe.conflict]', {
      userId: decision.user.id,
      driverId: decision.driver.id,
      source: params.source,
      maskedAccountId: maskStripeAccountId(account.id),
      fieldsReconciled: [],
      errorCode: 'STRIPE_ACCOUNT_CONFLICT',
    });
    return {
      ok: false,
      accountId: null,
  ...
  console.info('[driver.stripe.account_created]', {
    userId: decision.user.id,
    driverId: decision.driver.id,
    source: params.source,
    maskedAccountId: maskStripeAccountId(account.id),
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/auth/user', isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (user.role === 'driver') {
        ...
        if (driverData) {
          // Driver bank details are collected and stored by Stripe onboarding, not CreteXchange.
          const safeDriverData = { ...driverData } as Record<string, unknown>;
          delete safeDriverData.accountNumber;
          delete safeDriverData.routingNumber;
          delete safeDriverData.bankName;
          delete safeDriverData.accountHolderName;
          roleData = {
            ...safeDriverData,
            stripeBankDetailsManagedBy: 'stripe',
          };
        }
  ...
  app.get('/api/column/status', isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (user.role === 'driver') {
        ...
        const status = await canonicalDriverStripeService.getDriverStripeStatus({
          user,
          driver,
          source: 'GET /api/column/status',
        });
        console.info('[driver.stripe.status_checked]', {
          userId,
          driverId: driver.id,
          ...
          errorCode: status.errorState?.code || null,
        });
        return res.json(buildDriverColumnStatusResponse(status, driver.stripeTreasuryAccountId));
      }

      // Preserve the existing non-driver Column/Treasury response independently of Connect payout readiness.
      const owner = user.role === 'owner' ? await storage.getOwner(userId) : null;
      const bankAccountId = owner?.stripeTreasuryAccountId || null;
      return res.json({
        isOnboarded: Boolean(user.stripeConnectAccountId),
        entityId: user.stripeConnectAccountId || null,
        bankAccountId,
        accountLast4: null,
        requiresSetup: Boolean(user.stripeConnectAccountId && !bankAccountId),
      });
    } catch (error) {
  ...
  app.post('/api/column/onboard', isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (user.role === 'driver') {
        console.info('[driver.stripe.onboarding_resolved]', {
          userId,
          driverId: null,
          source: 'POST /api/column/onboard',
          legacyAdapter: true,
        });
  ...
  app.post('/api/drivers/bank-connect/complete', isAuthenticated, async (req: any, res) => {
    return res.status(410).json({
      message: 'Driver bank account details are collected only through Stripe secure onboarding.',
      reason: 'driver_bank_details_collected_by_stripe',
    });
  });
  ...
  async function handleDriverStripeOnboarding(req: any, res: any) {
    const source = 'GET /api/drivers/stripe-onboarding';
    try {
      ...
      if (!await isDriverStripePayoutsEnabled(userId)) {
        return res.status(403).json({
      ...
      const accountResult = await withDriverStripeOnboardingLock(userId, () =>
        safelyResolveDriverStripeOnboardingAccount({ user, driver, source }),
      );
      if (!accountResult.ok || !accountResult.accountId) {
        const errorCode = accountResult.resolution.errorState?.code || 'STRIPE_STATUS_UNAVAILABLE';
        return res.status(getDriverStripeOnboardingHttpStatus(errorCode)).json({
          message: accountResult.resolution.supportRequired
            ? 'Stripe payout setup needs support review before onboarding can continue.'
            : 'Stripe payout setup is temporarily unavailable. Please try again later.',
          code: errorCode,
          status: accountResult.resolution.status,
          discoveryOutcome: accountResult.resolution.discoveryOutcome,
          supportRequired: accountResult.resolution.supportRequired,
          accountIdPresent: accountResult.resolution.accountIdPresent,
          onboardingLinkGenerated: false,
        });
      }

      const [resolvedUser, resolvedDriver] = await Promise.all([
        storage.getUser(userId),
        storage.getDriver(userId),
      ]);
      if (!resolvedUser || !resolvedDriver) {
        return res.status(500).json({
          message: 'Stripe payout setup could not reload the driver record.',
          code: 'STRIPE_STATUS_UNAVAILABLE',
          onboardingLinkGenerated: false,
        });
      }
      const resolvedStatus = await canonicalDriverStripeService.getDriverStripeStatus({
        user: resolvedUser,
        driver: resolvedDriver,
        source: `${source}:post-resolution-status`,
      });
      if (resolvedStatus.status === 'status_unavailable' || resolvedStatus.status === 'account_conflict') {
        const errorCode = resolvedStatus.errorState?.code || 'STRIPE_STATUS_UNAVAILABLE';
        return res.status(getDriverStripeOnboardingHttpStatus(errorCode)).json({
          message: 'Stripe payout status could not be safely verified for onboarding.',
          code: errorCode,
          status: resolvedStatus.status,
          accountIdPresent: resolvedStatus.accountIdPresent,
          onboardingLinkGenerated: false,
        });
      }

      if (resolvedStatus.payoutReady) {
        console.info('[driver.stripe.onboarding_resolved]', {
          userId,
          driverId: driver.id,
          source,
          canonicalStatus: resolvedStatus.status,
          discoveryOutcome: accountResult.resolution.discoveryOutcome,
          selectedCandidateSource: accountResult.resolution.candidateSource,
          fieldsReconciled: accountResult.updatedFields,
          maskedAccountId: maskStripeAccountId(accountResult.accountId),
          accountCreated: accountResult.created,
        });
        return res.json({
          status: resolvedStatus.status,
          onboardingComplete: resolvedStatus.onboardingComplete,
          payoutReady: resolvedStatus.payoutReady,
          accountIdPresent: true,
          maskedAccountId: maskStripeAccountId(accountResult.accountId),
          accountCreated: accountResult.created,
          accountReused: accountResult.reused,
          discoveryOutcome: accountResult.resolution.discoveryOutcome,
          onboardingLinkGenerated: false,
        });
      }
      ...
      if (!accountLink?.url) {
        console.error('[driver.stripe.status_unavailable]', {
          ...
          source,
          maskedAccountId: maskStripeAccountId(accountResult.accountId),
          errorCode: 'STRIPE_ACCOUNT_LINK_MISSING_URL',
        });
  ...
  // Canonical driver-facing Stripe status. Normal reads never reconcile or write.
  app.get('/api/drivers/stripe-status', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const status = await canonicalDriverStripeService.getDriverStripeStatus({
        user,
        driver,
        source: 'GET /api/drivers/stripe-status',
      });
      console.info('[driver.stripe.status_checked]', {
        userId,
        driverId: driver.id,
      ...
      return res.json(buildDriverStripeStatusApiResponse(status));
    } catch (error) {
      console.error('[driver.stripe.status_unavailable]', {
        userId: req.user?.id || null,
        source: 'GET /api/drivers/stripe-status',
      ...
      return res.status(500).json({
        message: 'Stripe payout status is temporarily unavailable.',
        code: 'STRIPE_STATUS_UNAVAILABLE',
      });
    }
  });
  ...
  app.get('/api/drivers/stripe-requirements', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const status = await canonicalDriverStripeService.getDriverStripeStatus({
        user,
        driver,
        source: 'GET /api/drivers/stripe-requirements',
      });
      const { formatStripeRequirements } = await import('./stripeUtils');
      const currentlyDue = [...status.requirementsDue];
      ...
      return res.status(500).json({
        message: 'Stripe payout requirements are temporarily unavailable.',
        code: 'STRIPE_STATUS_UNAVAILABLE',
      });
    }
  });
  ...
  app.get('/api/owners/activities', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const activitiesWithGeofenceContext = activities.map((activity) => {
        ...
        return {
          ...stripPrivateOwnerActivityEvidence(activity as unknown as Record<string, unknown>),
          geofenceContext: ownerGeofenceEnabled
            ? projectOwnerGeofenceContext(evaluation)
            : undefined,
        };
      });
  ...
  app.put('/api/owners/activities/:id/verify', isAuthenticated, async (req: any, res) => {
    ...
    try {
      ...
      // `pending` is the only persisted state that may transition to `verified`.
      // Legacy/presentation aliases must not create a new operational transition.
  ...
  app.post('/api/drivers/request-debit-card', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Resolve Connect through the canonical onboarding-safe path before any Issuing work.
      if (!driver.stripeConnectAccountId) {
        try {
          const accountResult = await withDriverStripeOnboardingLock(userId, () =>
            safelyResolveDriverStripeOnboardingAccount({
              user,
              driver,
              source: 'POST /api/drivers/request-debit-card',
            }),
          );
          if (!accountResult.ok || !accountResult.accountId) {
            const errorCode = accountResult.resolution.errorState?.code || 'STRIPE_STATUS_UNAVAILABLE';
            return res.status(getDriverStripeOnboardingHttpStatus(errorCode)).json({
              message: 'Stripe payment account setup needs review before debit card setup can continue.',
              code: errorCode,
            });
          }
          driver.stripeConnectAccountId = accountResult.accountId;
        } catch (error: any) {
          console.error('[driver.stripe.status_unavailable]', {
            userId,
            driverId: driver.id,
            source: 'POST /api/drivers/request-debit-card',
            errorCode: error?.code || 'DRIVER_STRIPE_ACCOUNT_CREATE_FAILED',
            stripeError: getSafeStripeErrorDetails(error),
          });
          return res.status(typeof error?.statusCode === 'number' ? error.statusCode : 400).json({
            message: error?.statusCode === 400 && error?.message
              ? error.message
              : 'Stripe payment account setup failed. Please complete your profile details and try again.',
            code: error?.code || 'DRIVER_STRIPE_ACCOUNT_CREATE_FAILED',
            missingFields: error?.missingFields || [],
          });
        }
      }
    ...
    try {
      ...
      try {
        await stripeService.updateConnectedAccountWithCompleteInfo(
          ...
          { payoutOnly: targetUser.role === 'driver' }
        );
  ...
  app.get('/api/admin/dashboard', isAuthenticated, async (req: any, res) => {
    try {
      ...
      res.json({
        ...
        dashboardMeta: {
          ...
          coreSources: {
            ...
            billingReceivables: "washout_activities.fee_cents_platform + billing batches",
          },
          weekStatsWindow: {
            days: 7,
          ...
          readsImmediateBillingReceivables: true,
        },
      ...
      console.log("[ADMIN_DASHBOARD] response summary", {
        ...
        platformWashoutPaidRevenue: weekResult?.platformWashoutPaidRevenue,
    ...
    try {
      ...
      for (const driverData of allDrivers) {
        try {
          ...
          // Update account to request the driver connected-account payout capability.
  ...
  app.get('/api/drivers/lottery-history', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const history = await storage.getDriverLotteryHistory(driver.id);
      res.json(history);
    } catch (error: any) {
      console.error("Error fetching driver lottery history:", error);
      res.status(500).json({ message: error.message || "Failed to fetch lottery history" });
    }
  });
  ...
  const isCompletedLotteryDrawing = (drawing: any, winnerRows: any[]) => {
    ...
    const winnerNotificationCount = Number(drawing.winnerNotificationCount || 0);
    const participantNotificationCount = Number(drawing.participantNotificationCount || 0);
    return winnerRows.length > 0 && winnerNotificationCount > 0 && participantNotificationCount > 0;
  };

  const createLotteryNotificationOnceTx = async (tx: any, notification: {
  ...
  app.post('/api/admin/lottery/execute', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const existingWinners = existing ? await storage.getLotteryDrawingWinners(existing.id) : [];
      const existingDrawingIsComplete = isCompletedLotteryDrawing(existing, existingWinners);
      if (existing && existingDrawingIsComplete) {
        return res.status(409).json({
      ...
      const result = await db.transaction(async (tx) => {
        if (partialExistingDrawing) {
          const existingNotificationRows = await tx
            .select({
              notificationId: lotteryNotifications.notificationId,
            })
            .from(lotteryNotifications)
            .where(eq(lotteryNotifications.lotteryDrawingId, partialExistingDrawing.id));
        ...
        const [drawing] = await tx.insert(lotteryDrawings).values({
          lotteryMonth: month,
          lotteryYear: year,
          executedBy: user.id,
          firstPlaceDriverId: winners[0]?.driverId || null,
          firstPlaceDriverName: winners[0]?.driverName || null,
          firstPlaceTicketNumber: winners[0]?.ticketNumber || null,
          firstPlacePayoutPreference: winners[0]?.payoutPreference || null,
          firstPrize: winners[0]?.prize || prizeConfigs[0]?.title || null,
          secondPlaceDriverId: winners[1]?.driverId || null,
          secondPlaceDriverName: winners[1]?.driverName || null,
          secondPlaceTicketNumber: winners[1]?.ticketNumber || null,
          secondPlacePayoutPreference: winners[1]?.payoutPreference || null,
          secondPrize: winners[1]?.prize || prizeConfigs[1]?.title || null,
          thirdPlaceDriverId: winners[2]?.driverId || null,
          thirdPlaceDriverName: winners[2]?.driverName || null,
          thirdPlaceTicketNumber: winners[2]?.ticketNumber || null,
          thirdPlacePayoutPreference: winners[2]?.payoutPreference || null,
          thirdPrize: winners[2]?.prize || prizeConfigs[2]?.title || null,
        }).returning();

        if (!drawing) {
          throw new Error("Unable to create lottery drawing record");
        }
        ...
        for (const [driverId, groupedWinners] of Array.from(winnerGroups.entries())) {
          ...
          if (!driver) {
            throw new Error(`Driver not found for winner ${driverId}`);
          }
          const { title, message } = buildLotteryWinnerGroupMessage(groupedWinners, monthName, year);
          const result = await createLotteryNotificationOnceTx(tx, {
        ...
        for (const driverId of participantDriverIds) {
          const driver = driverMap.get(driverId);
          if (!driver) {
            throw new Error(`Driver not found for participant ${driverId}`);
          }
          const result = await createLotteryNotificationOnceTx(tx, {
            lotteryDrawingId: drawing.id,
            lotteryMonth: month,
        ...
        return {
          ...
          participantNotificationCount,
        };
      });

      console.log(`🎉 Monthly prize drawing executed for ${monthName} ${year} by ${user.username}. Winners: ${result.winners.map(w => w.driverName).join(', ')}`);
      console.log(`📣 Monthly prize drawing notifications sent for drawing ${result.drawing.id}: ${result.winnerNotificationCount} winner messages, ${result.participantNotificationCount} participant announcements`);

      res.json({
        message: `Monthly prize drawing complete! ${result.winners.length} winner${result.winners.length !== 1 ? 's' : ''} selected and notified.`,
        drawing: result.summary ? { ...result.drawing, ...result.summary } : result.drawing,
        winners: result.winners.map((w) => ({ ...w })),
      });
    } catch (error: any) {
  ...
  app.get("/api/admin/analytics/network/overview", isAuthenticated, async (req: any, res: any) => {
    try {
      const user = await storage.getUser(req.user.id);
      if (!user || !canAccessPlatformAnalytics(user.role)) {
        return res.status(403).json({ message: "Network intelligence access required" });
      }
      return res.json(await buildNetworkIntelligence(db, parseNetworkIntelligenceQuery(req.query || {})));
    } catch (error) {
      if (error instanceof PlatformAnalyticsQueryError) return res.status(400).json({ message: error.message });
      console.error("Error aggregating network intelligence:", error);
      return res.status(500).json({ message: "Failed to aggregate network intelligence" });
    }
  });
  ...
  app.post('/api/stripe/connect/create-account', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const source = 'POST /api/stripe/connect/create-account';
      const accountResult = await withDriverStripeOnboardingLock(userId, () =>
        safelyResolveDriverStripeOnboardingAccount({ user, driver, source }),
      );
      if (!accountResult.ok || !accountResult.accountId) {
        ...
        return res.status(getDriverStripeOnboardingHttpStatus(errorCode)).json({
          message: accountResult.resolution.supportRequired
            ? 'Stripe payout setup needs support review before account setup can continue.'
            : 'Stripe payout account setup is temporarily unavailable.',
          code: errorCode,
          status: accountResult.resolution.status,
          discoveryOutcome: accountResult.resolution.discoveryOutcome,
          accountIdPresent: accountResult.resolution.accountIdPresent,
      ...
      console.error('[driver.stripe.status_unavailable]', {
        userId: req.user?.id || null,
        source: 'POST /api/stripe/connect/create-account',
        errorCode: error?.code || 'DRIVER_STRIPE_ACCOUNT_CREATE_FAILED',
        stripeError: getSafeStripeErrorDetails(error),
      });
      return res.status(statusCode).json({
        message: statusCode === 400 && error?.message
          ? error.message
          : 'Failed to create or reuse the connected account.',
        code: error?.code || 'DRIVER_STRIPE_ACCOUNT_CREATE_FAILED',
        missingFields: error?.missingFields || [],
        invalidFields: error?.invalidFields || [],
      });
  ...
  // Deprecated independent status logic: retain this route as a thin canonical-service adapter.
  app.get('/api/stripe/connect/account-status', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const status = await canonicalDriverStripeService.getDriverStripeStatus({
        user,
        driver,
        source: 'GET /api/stripe/connect/account-status',
      });
      console.info('[driver.stripe.status_checked]', {
        userId,
        driverId: driver.id,
      ...
      return res.json(buildLegacyDriverStripeAccountStatusResponse(status));
    } catch (error) {
      console.error('[driver.stripe.status_unavailable]', {
        userId: req.user?.id || null,
        source: 'GET /api/stripe/connect/account-status',
  ...
  app.post('/api/activities/create-with-photos', isAuthenticated, driverOperationalReadinessMiddleware, async (req: any, res) => {
    try {
      ...
      const geofenceEvidenceCaptureEnabled = shouldCaptureSubmissionGeofenceEvidence({
        advisoryEnabled: geofenceAdvisoryEnabled,
        enforcementEnabled: geofenceEnforcementEnabled,
      });
      if (geofenceEvidenceCaptureEnabled) {
        ...
        const geofenceRouting = resolveSubmissionGeofenceRouting(geofenceDecision, geofenceEnforcementEnabled);
        if (geofenceRouting.recover && geofenceDecision.action === "recover") {
            return res.status(422).json({
        ...
        geofenceEvaluationEvidence = submissionGeofenceService.prepareActivityEvaluation({
          ...
          evidenceComplete: Boolean(
            geofenceResult.observationTimestamp
            && geofenceResult.accuracyMeters !== null
            && photoData.length > 0
            && (
              geofenceResult.state !== "OUTSIDE_BOUNDARY_WITHIN_EXCEPTION_ZONE"
              || parsedGeofence.data.acknowledgement?.confirmed
            )
          ),
        }).evidence;
      }
      ...
      if (!result.reused && result.photos.length > 0) {
        const objectStorageService = new ObjectStorageService();
        const aclPolicy: ObjectAclPolicy = {
          owner: userId,
          visibility: "private",
      ...
      res.json({
        ...
        photoCount: result.photos.length,
        geofence: result.geofenceEvaluation ? {
          state: result.geofenceEvaluation.resultState,
          boundaryVersionId: result.geofenceEvaluation.boundaryVersionId,
          exceptionAccepted: Boolean(result.geofenceEvaluation.exceptionAcknowledgementCode),
        } : null,
