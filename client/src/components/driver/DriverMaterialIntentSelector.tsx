...
function localizedMaterialName(material: DriverMaterial, t: (key: string) => string) {
  const key = `driver.material.system.${material.slug}`;
  const translated = t(key);
  return translated === key ? material.displayName : translated;
}

function localizedMaterialCategory(category: string | null | undefined, t: (key: string) => string) {
  if (!category) return null;
  const categoryKey = `driver.material.category.${category.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "")}`;
