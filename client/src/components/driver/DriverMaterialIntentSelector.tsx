...
export function DriverMaterialIntentSelector({ compact = false }: { compact?: boolean }) {
  ...
  if (compact && !expanded) return <Card className="border-border/70 bg-card/90"><CardContent className="flex items-center justify-between gap-3 p-3"><div><p className="text-xs font-semibold uppercase text-muted-foreground">{t("driver.material.active")}</p><p className="font-medium">{active?.displayName || t("driver.material.none")}</p></div><Button variant="outline" size="sm" onClick={() => setExpanded(true)} data-testid="button-change-active-material">{t("driver.material.change")}</Button></CardContent></Card>;

  return <Card className="border-border/70 bg-card/90" data-testid="driver-material-intent-selector"><CardContent className="space-y-3 p-4"><div><p className="font-semibold">{t("driver.material.question")}</p><p className="text-sm text-muted-foreground">{t("driver.material.help")}</p></div>{catalog.isLoading || intent.isLoading ? <p className="text-sm text-muted-foreground">{t("common.loading")}</p> : <><div className="relative"><Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><Input className="pl-9" value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t("driver.material.search")} data-testid="input-driver-material-search" /></div><div className="grid max-h-64 grid-cols-1 gap-2 overflow-y-auto sm:grid-cols-2">{visible.map((material) => <Button key={material.slug} type="button" variant={active?.slug === material.slug ? "default" : "outline"} className="h-auto justify-start whitespace-normal p-3 text-left" disabled={save.isPending} onClick={() => save.mutate(material.slug)} data-testid={`button-driver-material-${material.slug}`}><span className="flex w-full items-start justify-between gap-2"><span><span className="block font-semibold">{material.displayName}</span>{material.category && <span className="block text-xs opacity-75">{material.category}</span>}</span>{active?.slug === material.slug && <Check className="h-4 w-4 shrink-0" />}</span></Button>)}</div>{visible.length === 0 && <p className="text-sm text-muted-foreground">{t("driver.material.noMatches")}</p>}{save.isError && <p className="text-sm text-destructive">{t("driver.material.saveFailed")}</p>}{save.isSuccess && <p className="text-sm text-emerald-600">{t("driver.material.saved")}</p>}</>}</CardContent></Card>;
}
