...
function geofencePresentationKey(state: string, reasonCode: string): string {
  if (state === "OUTSIDE_BOUNDARY_WITHIN_EXCEPTION_ZONE") return "yellow";
  if (state === "LOCATION_UNAVAILABLE") return "gray.gpsUnavailable";
  if (state === "LOCATION_ACCURACY_INSUFFICIENT") {
    if (reasonCode === "LOCATION_UNCERTAINTY_OVERLAPS_BOUNDARY") return "gray.nearBoundary";
    if (reasonCode === "LOCATION_UNCERTAINTY_OVERLAPS_EXCEPTION_THRESHOLD") return "gray.nearAdvisoryLimit";
    return "gray.accuracyInsufficient";
  }
  if (state === "GEOMETRY_UNAVAILABLE") return "gray.boundaryUnavailable";
  if (state === "GEOMETRY_INVALID") return "gray.boundaryInvalid";
  return "unavailable";
}

function AdminGeofenceContextCard({ activityId }: { activityId: string }) {
  const { t, language } = useLanguage();
  const context = useQuery<{
    state: string; reasonCode: string; boundaryVersion: number | null; acknowledgementCode: string | null;
    driverNote: string | null; evidenceComplete: boolean; evaluatedAt: string;
  }>({
    queryKey: ["/api/admin/geofence/activities", activityId, "context"],
    queryFn: async () => (await apiRequest("GET", `/api/admin/geofence/activities/${activityId}/context`)).json(),
    retry: false,
  });
  if (context.isLoading) return <Card><CardContent className="p-4 text-sm" role="status">{t("photoReview.geofenceContextLoading")}</CardContent></Card>;
  if (context.isError || !context.data) return <Card><CardContent className="p-4 text-sm text-muted-foreground" role="alert">{t("photoReview.geofenceContextUnavailable")}</CardContent></Card>;
  const presentationKey = geofencePresentationKey(context.data.state, context.data.reasonCode);
  return <Card data-testid="admin-geofence-operational-context"><CardHeader><CardTitle className="text-base">{t("geofence.admin.context")}</CardTitle></CardHeader><CardContent className="space-y-2 text-sm">
    <p><strong>{t("common.status")}:</strong> {t(`geofence.admin.presentation.${presentationKey}`)}</p>
    <p>{t(`geofence.admin.guidance.${presentationKey}`)}</p>
    <p><strong>{t("geofence.admin.boundaryVersion")}:</strong> {context.data.boundaryVersion || "—"}</p>
    {context.data.acknowledgementCode && <p><strong>{t("geofence.admin.reasonCode")}:</strong> {t(`geofence.reason.${context.data.acknowledgementCode}`)}</p>}
    {context.data.driverNote && <p><strong>{t("geofence.driver.note")}:</strong> {context.data.driverNote}</p>}
    <p className="text-xs text-muted-foreground">{new Date(context.data.evaluatedAt).toLocaleString(localeForLanguage(language))}</p>
  </CardContent></Card>;
}
...
export default function AdminPhotoReview() {
  ...
  const initialQuery = useMemo(() => new URLSearchParams(typeof window === "undefined" ? "" : window.location.search), []);
  const linkedActivityId = initialQuery.get("activityId") || "";
  const initialView = initialQuery.get("view");
  const [view, setView] = useState<ReviewView>(reviewViews.includes(initialView as ReviewView) ? initialView as ReviewView : "needs_review");
  ...
  const [linkedActivityOpened, setLinkedActivityOpened] = useState(false);
  const linkedActivityTriggerRef = useRef<HTMLButtonElement | null>(null);
  ...
  if (linkedActivityId) query.set("activityId", linkedActivityId);
  ...
  useEffect(() => {
    if (!linkedActivityId || linkedActivityOpened || !queue.isSuccess) return;
    const linkedItem = currentItems.find((item) => item.submission.id === linkedActivityId);
    if (!linkedItem) return;
    setSelected(linkedItem);
    setDecision(null);
    setReason("");
    setConfirmed(false);
    setLinkedActivityOpened(true);
  }, [currentItems, linkedActivityId, linkedActivityOpened, queue.isSuccess]);

  const closeEvidence = () => {
    if (decide.isPending) return;
    const shouldRestoreLinkedFocus = selected?.submission.id === linkedActivityId;
    setSelected(null);
    if (shouldRestoreLinkedFocus) requestAnimationFrame(() => linkedActivityTriggerRef.current?.focus());
  };
