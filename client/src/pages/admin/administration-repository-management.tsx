...
type RefreshSnapshot = { state: "idle" | "running" | "completed" | "completed_with_warnings" | "failed"; startedAt: string | null; completedAt: string | null; durationMs: number | null; documentsDiscovered: number; documentsSynchronized: number; warnings: number; errors: number; sourceCommit: string | null };
type RefreshResult = { status: "completed" | "failed"; report: { inventoryGenerationStatus: string; documentsDiscovered: number; documentsSynchronized: number }; warnings: Array<{ code: string; message: string }>; errors: Array<{ code: string; message: string }> };
...
const publicErrorMessage = (error: unknown) => {
  ...
  if (/administration_repository_(environment_identity_invalid|source_commit_invalid|production_authorization_missing|production_authorization_mismatch)/.test(message)) return "Documentation refresh is not authorized for this environment. The current library remains available.";
...
export default function AdministrationRepositoryManagement() {
  ...
  return <ManagementShell>
    ...
    <section className="grid gap-4 lg:grid-cols-2"><Card><CardHeader><CardTitle className="flex items-center gap-2"><ShieldCheck className="h-5 w-5" />Latest refresh</CardTitle><CardDescription>Refresh status and verified inventory-generation outcome from the protected synchronization API.</CardDescription></CardHeader><CardContent className="space-y-3">{activeStatus ? <><StatusBadge state={activeStatus.state} /><dl className="grid gap-3 text-sm sm:grid-cols-2"><Detail label="Started" value={displayDate(activeStatus.startedAt)} /><Detail label="Completed" value={displayDate(activeStatus.completedAt)} /><Detail label="Duration" value={displayDuration(activeStatus.durationMs)} /><Detail label="Source commit" value={activeStatus.sourceCommit?.slice(0, 12) || "—"} /></dl><dl className="grid gap-3 text-sm sm:grid-cols-2"><Detail label="Documents discovered" value={String(activeStatus.documentsDiscovered)} /><Detail label="Documents synchronized" value={String(activeStatus.documentsSynchronized)} /><Detail label="Warnings" value={String(activeStatus.warnings)} /><Detail label="Errors" value={String(activeStatus.errors)} /></dl></> : <p className="text-sm text-muted-foreground">No refresh has been recorded in this browser session. Review recent history for persisted results.</p>}</CardContent></Card><Card><CardHeader><CardTitle className="flex items-center gap-2"><Clock3 className="h-5 w-5" />Last synchronized</CardTitle><CardDescription>Use refresh only after governed documentation changes are deployed.</CardDescription></CardHeader><CardContent className="space-y-3"><StatusBadge state={status.data?.latest?.status || "idle"} /><p className="text-sm"><span className="font-medium">Last activity:</span> {displayDate(status.data?.latest?.completedAt || status.data?.latest?.startedAt || health.data?.latestSynchronization?.completedAt || health.data?.latestSynchronization?.startedAt)}</p><p className="text-sm text-muted-foreground">The active Documentation Library remains available if validation or inventory generation fails.</p></CardContent></Card></section>
