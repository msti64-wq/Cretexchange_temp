...
import {
  ...
  normalizeFinancialWorkspaceDate,
...
type CanonicalSummary = { missingObligations: { count: number }; openCanonicalObligations: CanonicalSummaryMetric; draftBatches: CanonicalSummaryMetric; readyForReview: CanonicalSummaryMetric; approvedNotExecuted: CanonicalSummaryMetric; exceptions: { count: number } };
type FinancialSchemaCapabilities = { previewAvailable: boolean; creationAvailable: boolean; auditSchemaAvailable: boolean; obligationKindAvailable: boolean; canonicalPartialIndexAvailable: boolean; globalActivityIndexPresent: boolean; schemaState: string; creationUnavailableReason: string | null };
type BatchDetail = {
...
type BatchSelectionPreview = { eligibleCount: number; excludedCount: number; alreadyBatchedCount: number; ineligibleCount: number; frozenDriverIncentiveCents: number; frozenPlatformFeeCents: number; facilityChargeCents?: number; frozenFacilityChargeCents: number; selectionHash: string };
...
export default function AdminFinancialWorkspace() {
  ...
  const draftFacilityId = action?.type === "draft" ? normalizeFinancialWorkspaceReference(recordReference) : null;
  const validRange = Boolean(normalizeFinancialWorkspaceDate(fromDate) && normalizeFinancialWorkspaceDate(throughDate) && fromDate <= throughDate);
  const batchPreview = useQuery<BatchSelectionPreview>({
    queryKey: ["/api/admin/financial-batches/preview", draftFacilityId, fromDate, throughDate],
    enabled: allowed && action?.type === "draft" && Boolean(draftFacilityId) && validRange,
    queryFn: async () => (await apiRequest("POST", "/api/admin/financial-batches/preview", { facilityId: draftFacilityId, fromDate, throughDate })).json(),
  });
  ...
  const validationMessage = () => {
    ...
    if (action.type === "draft") {
      ...
      if (!validRange) return "financialWorkspace.validation.periodInvalid";
      if (batchPreview.isLoading || batchPreview.isError || !batchPreview.data) return "financialWorkspace.validation.previewUnavailable";
      if (!batchPreview.data.eligibleCount) return "financialWorkspace.validation.previewUnavailable";
    }
  ...
  const mutation = useMutation({
    ...
    mutationFn: async () => {
      ...
      if (action.type === "draft") {
        return (await apiRequest("POST", "/api/admin/financial-batches", { facilityId: reference!, fromDate, throughDate, selectionHash: batchPreview.data?.selectionHash, reason: reason.trim(), idempotencyKey: `workspace-${reference!}-${fromDate}-${throughDate}-${batchPreview.data?.selectionHash}` })).json();
      }
  ...
  return <div className="min-h-screen bg-background pb-24">
    ...
    <ActionDialog t={t} action={action} reason={reason} reasonCategory={reasonCategory} preview={obligationPreview.data} previewLoading={obligationPreview.isLoading} previewUnavailable={obligationPreview.isError || !obligationPreview.data} batchPreview={batchPreview.data} batchPreviewLoading={batchPreview.isLoading} batchPreviewUnavailable={batchPreview.isError || !batchPreview.data} creationUnavailable={action?.type === "obligation" && !capabilities.data?.creationAvailable} creationUnavailableReason={capabilities.data?.creationUnavailableReason || null} reference={recordReference} fromDate={fromDate} throughDate={throughDate} confirmed={approvedCancellationConfirmed} category={cancellationCategory} validationKey={validationKey} pending={mutation.isPending} referenceInput={referenceInput} reasonInput={reasonInput} onReason={setReason} onReasonCategory={setReasonCategory} onReference={setRecordReference} onFromDate={setFromDate} onThroughDate={setThroughDate} onConfirmed={setApprovedCancellationConfirmed} onCategory={setCancellationCategory} onClose={clearAction} onRetryPreview={() => void obligationPreview.refetch()} onRetryBatchPreview={() => void batchPreview.refetch()} onSubmit={() => mutation.mutate()} />
...
function ActionDialog({ t, action, reason, reasonCategory, preview, previewLoading, previewUnavailable, batchPreview, batchPreviewLoading, batchPreviewUnavailable, creationUnavailable, creationUnavailableReason, reference, fromDate, throughDate, confirmed, category, validationKey, pending, referenceInput, reasonInput, onReason, onReasonCategory, onReference, onFromDate, onThroughDate, onConfirmed, onCategory, onClose, onRetryPreview, onRetryBatchPreview, onSubmit }: { t: Translate; action: Action | null; reason: string; reasonCategory: string; preview?: { driverIncentiveCents: number; platformFeeCents: number; facilityChargeCents: number }; previewLoading: boolean; previewUnavailable: boolean; batchPreview?: BatchSelectionPreview; batchPreviewLoading: boolean; batchPreviewUnavailable: boolean; creationUnavailable: boolean; creationUnavailableReason: string | null; reference: string; fromDate: string; throughDate: string; confirmed: boolean; category: string; validationKey: string | null; pending: boolean; referenceInput: React.RefObject<HTMLInputElement>; reasonInput: React.RefObject<HTMLTextAreaElement>; onReason: (value: string) => void; onReasonCategory: (value: string) => void; onReference: (value: string) => void; onFromDate: (value: string) => void; onThroughDate: (value: string) => void; onConfirmed: (value: boolean) => void; onCategory: (value: string) => void; onClose: () => void; onRetryPreview: () => void; onRetryBatchPreview: () => void; onSubmit: () => void }) {
  if (!action) return null;
    ...
    <DialogContent className="max-h-[90vh] overflow-y-auto" onEscapeKeyDown={(event) => { if (pending) event.preventDefault(); }} onInteractOutside={(event) => { if (pending) event.preventDefault(); }}>
      ...
      <div className="space-y-4">
        ...
        {referenceAction && <>
          ...
          {action.type === "draft" && <><div className="grid gap-3 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="workspace-from-date">{t("financialWorkspace.fromDate")}</Label><Input id="workspace-from-date" type="date" value={fromDate} onChange={(event) => onFromDate(event.target.value)} aria-invalid={periodError} aria-describedby={periodError ? "workspace-action-error" : undefined} /></div><div className="space-y-2"><Label htmlFor="workspace-through-date">{t("financialWorkspace.throughDate")}</Label><Input id="workspace-through-date" type="date" value={throughDate} onChange={(event) => onThroughDate(event.target.value)} aria-invalid={periodError} aria-describedby={periodError ? "workspace-action-error" : undefined} /></div></div><p className="text-xs text-muted-foreground">{t("financialWorkspace.serviceDateHelp")}</p><section className="rounded-lg border bg-muted/30 p-3">{batchPreviewLoading ? <LoadingRows t={t} /> : batchPreviewUnavailable ? <div role="alert" className="space-y-2"><p>{t("financialWorkspace.preview.unavailable")}</p><Button type="button" size="sm" variant="outline" onClick={onRetryBatchPreview}>{t("financialWorkspace.preview.retry")}</Button></div> : <RecordGrid values={[[t("financialWorkspace.preview.eligible"), String(batchPreview?.eligibleCount || 0)], [t("financialWorkspace.preview.excluded"), String(batchPreview?.excludedCount || 0)], [t("financialWorkspace.preview.alreadyBatched"), String(batchPreview?.alreadyBatchedCount || 0)], [t("financialWorkspace.preview.ineligible"), String(batchPreview?.ineligibleCount || 0)], [t("financialWorkspace.summary.driver"), formatFinancialWorkspaceCents(batchPreview?.frozenDriverIncentiveCents ?? null)], [t("financialWorkspace.summary.fee"), formatFinancialWorkspaceCents(batchPreview?.frozenPlatformFeeCents ?? null)], [t("financialWorkspace.summary.facility"), formatFinancialWorkspaceCents(batchPreview?.frozenFacilityChargeCents ?? null)]]} />}</section></>}
        </>}
        {approvedCancel && <>
