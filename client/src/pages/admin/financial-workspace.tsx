...
function ActionDialog({ t, action, reason, reasonCategory, preview, previewLoading, previewUnavailable, reference, periodAnchor, confirmed, category, validationKey, pending, referenceInput, reasonInput, onReason, onReasonCategory, onReference, onPeriod, onConfirmed, onCategory, onClose, onSubmit }: { t: Translate; action: Action | null; reason: string; reasonCategory: string; preview?: { driverIncentiveCents: number; platformFeeCents: number; facilityChargeCents: number }; previewLoading: boolean; previewUnavailable: boolean; reference: string; periodAnchor: string; confirmed: boolean; category: string; validationKey: string | null; pending: boolean; referenceInput: React.RefObject<HTMLInputElement>; reasonInput: React.RefObject<HTMLTextAreaElement>; onReason: (value: string) => void; onReasonCategory: (value: string) => void; onReference: (value: string) => void; onPeriod: (value: string) => void; onConfirmed: (value: boolean) => void; onCategory: (value: string) => void; onClose: () => void; onSubmit: () => void }) {
  if (!action) return null;
  ...
  return <Dialog open onOpenChange={(open) => !open && onClose()}>
    <DialogContent className="max-h-[90vh] overflow-y-auto">
      ...
      <div className="space-y-4">
        {action.type === "obligation" && <><section className="rounded-lg border bg-muted/30 p-3"><span className="rounded-full border px-2 py-1 text-xs font-medium">{t("financialWorkspace.obligation.type")}</span><div className="mt-3">{previewLoading ? <LoadingRows t={t} /> : <RecordGrid values={[[t("financialWorkspace.label.verifiedActivity"), itemText(action.item || {}, "activityReference", unavailable(t))], [t("financialWorkspace.label.facility"), participantText(action.item || {}, "facility", unavailable(t))], [t("financialWorkspace.label.driver"), participantText(action.item || {}, "driver", unavailable(t))], [t("financialWorkspace.label.reviewTimestamp"), itemText(action.item || {}, "verificationTimestamp", unavailable(t))], [t("financialWorkspace.summary.driver"), preview ? formatFinancialWorkspaceCents(preview.driverIncentiveCents) : t("financialWorkspace.unavailableCanonical")], [t("financialWorkspace.summary.fee"), preview ? formatFinancialWorkspaceCents(preview.platformFeeCents) : t("financialWorkspace.unavailableCanonical")], [t("financialWorkspace.summary.facility"), preview ? formatFinancialWorkspaceCents(preview.facilityChargeCents) : t("financialWorkspace.unavailableCanonical")]]} />}</div></section><div className="space-y-2"><Label htmlFor="workspace-reason-category">{t("financialWorkspace.reasonCategory")}</Label><select id="workspace-reason-category" value={reasonCategory} onChange={(event) => onReasonCategory(event.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="missing_canonical_obligation">{t("financialWorkspace.reason.missingCanonical")}</option></select><p className="text-sm text-muted-foreground">{t("financialWorkspace.reason.guidance")}</p></div></>}
        {referenceAction && <>
