...
type Action = { type: "obligation" | "draft" | FinancialWorkspaceAction; batch?: FinancialBatchProjection };
type BatchDetail = {
...
export default function AdminFinancialWorkspace() {
  ...
  const clearAction = () => {
    setAction(null); setReason(""); setRecordReference(""); setApprovedCancellationConfirmed(false); setCancellationCategory(""); setValidationKey(null);
  };
  const validationMessage = () => {
    if (!action) return null;
    if (action.type === "obligation" || action.type === "draft") {
  ...
  const mutation = useMutation({
    ...
    mutationFn: async () => {
      ...
      if (action.type === "obligation") return (await apiRequest("POST", `/api/admin/financial-obligations/activities/${encodeURIComponent(reference!)}`, { reason: reason.trim() })).json();
  ...
  return <div className="min-h-screen bg-background pb-24">
    ...
    <ActionDialog t={t} action={action} reason={reason} reference={recordReference} periodAnchor={periodAnchor} confirmed={approvedCancellationConfirmed} category={cancellationCategory} validationKey={validationKey} pending={mutation.isPending} referenceInput={referenceInput} reasonInput={reasonInput} onReason={setReason} onReference={setRecordReference} onPeriod={setPeriodAnchor} onConfirmed={setApprovedCancellationConfirmed} onCategory={setCancellationCategory} onClose={clearAction} onSubmit={() => mutation.mutate()} />
...
function QueueCard({ values, actionLabel, onAction, testId }: { t: Translate; values: Array<[string, string]>; actionLabel: string; onAction: () => void; testId: string }) { return <Card><CardContent className="space-y-3 p-4"><RecordGrid values={values} /><div className="flex flex-wrap justify-end border-t pt-3"><Button size="sm" onClick={onAction} data-testid={testId}>{actionLabel}</Button></div></CardContent></Card>; }
...
function ActionDialog({ t, action, reason, reference, periodAnchor, confirmed, category, validationKey, pending, referenceInput, reasonInput, onReason, onReference, onPeriod, onConfirmed, onCategory, onClose, onSubmit }: { t: Translate; action: Action | null; reason: string; reference: string; periodAnchor: string; confirmed: boolean; category: string; validationKey: string | null; pending: boolean; referenceInput: React.RefObject<HTMLInputElement>; reasonInput: React.RefObject<HTMLTextAreaElement>; onReason: (value: string) => void; onReference: (value: string) => void; onPeriod: (value: string) => void; onConfirmed: (value: boolean) => void; onCategory: (value: string) => void; onClose: () => void; onSubmit: () => void }) {
  if (!action) return null;
  const referenceAction = action.type === "obligation" || action.type === "draft";
  const approvedCancel = action.type === "cancel" && action.batch?.state === "approved";
  const dialogTitle = action.type === "obligation" ? t("financialWorkspace.action.createObligation") : action.type === "draft" ? t("financialWorkspace.action.createDraft") : approvedCancel ? t("financialWorkspace.action.cancelNonExecuted") : t(actionKey(action.type));
  ...
  return <Dialog open onOpenChange={(open) => !open && onClose()}>
    <DialogContent className="max-h-[90vh] overflow-y-auto">
      <DialogHeader><DialogTitle>{dialogTitle}</DialogTitle><DialogDescription>{referenceAction ? t("financialWorkspace.temporary.notice") : t("financialWorkspace.detail.description")}</DialogDescription></DialogHeader>
      <div className="space-y-4">
        ...
        <div className="space-y-2"><Label htmlFor="workspace-reason">{t("financialWorkspace.operationalReason")}</Label><Textarea ref={reasonInput} id="workspace-reason" value={reason} onChange={(event) => onReason(event.target.value)} maxLength={500} aria-invalid={reasonError} aria-describedby={reasonError ? "workspace-action-error" : undefined} placeholder={t("financialWorkspace.reasonPlaceholder")} /></div>
      ...
      <DialogFooter><Button variant="outline" onClick={onClose}>{t("financialWorkspace.action.close")}</Button><Button onClick={onSubmit} disabled={Boolean(disabledKey)} aria-describedby={disabledKey ? "workspace-action-disabled" : undefined}>{pending && <Loader2 className="animate-spin" />}{dialogTitle}</Button></DialogFooter>
    </DialogContent>
  </Dialog>;
}
