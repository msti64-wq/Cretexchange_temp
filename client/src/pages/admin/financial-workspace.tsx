...
type FinancialSchemaCapabilities = { previewAvailable: boolean; creationAvailable: boolean; auditSchemaAvailable: boolean; obligationKindAvailable: boolean };
type BatchDetail = {
...
export default function AdminFinancialWorkspace() {
  ...
  const validationMessage = () => {
    ...
    if (action.type === "obligation") {
      ...
      if (!capabilities.data?.creationAvailable) return "financialWorkspace.validation.auditSchemaUnavailable";
  ...
  return <Dialog open onOpenChange={(open) => !open && !pending && onClose()}>
    <DialogContent className="max-h-[90vh] overflow-y-auto" onEscapeKeyDown={(event) => { if (pending) event.preventDefault(); }} onInteractOutside={(event) => { if (pending) event.preventDefault(); }}>
      ...
      <div className="space-y-4">
        {action.type === "obligation" && <><section className="rounded-lg border bg-muted/30 p-3"><span className="rounded-full border px-2 py-1 text-xs font-medium">{t("financialWorkspace.obligation.type")}</span><div className="mt-3">{previewLoading ? <LoadingRows t={t} /> : previewUnavailable ? <div role="alert" aria-live="assertive" className="space-y-3 rounded-md border border-amber-500/30 bg-amber-500/10 p-3 text-sm"><p>{t("financialWorkspace.preview.unavailable")}</p><Button type="button" size="sm" variant="outline" onClick={onRetryPreview}>{t("financialWorkspace.preview.retry")}</Button></div> : <RecordGrid values={[[t("financialWorkspace.label.verifiedActivity"), itemText(action.item || {}, "activityReference", unavailable(t))], [t("financialWorkspace.label.facility"), participantText(action.item || {}, "facility", unavailable(t))], [t("financialWorkspace.label.driver"), participantText(action.item || {}, "driver", unavailable(t))], [t("financialWorkspace.label.reviewTimestamp"), itemText(action.item || {}, "verificationTimestamp", unavailable(t))], [t("financialWorkspace.summary.driver"), preview ? formatFinancialWorkspaceCents(preview.driverIncentiveCents) : t("financialWorkspace.unavailableCanonical")], [t("financialWorkspace.summary.fee"), preview ? formatFinancialWorkspaceCents(preview.platformFeeCents) : t("financialWorkspace.unavailableCanonical")], [t("financialWorkspace.summary.facility"), preview ? formatFinancialWorkspaceCents(preview.facilityChargeCents) : t("financialWorkspace.unavailableCanonical")]]} />}</div></section>{creationUnavailable && <p role="alert" className="rounded-md border border-amber-500/30 bg-amber-500/10 p-3 text-sm">{t("financialWorkspace.auditSchemaUnavailable")}</p>}<div className="space-y-2"><Label htmlFor="workspace-reason-category">{t("financialWorkspace.reasonCategory")}</Label><select id="workspace-reason-category" value={reasonCategory} onChange={(event) => onReasonCategory(event.target.value)} className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm"><option value="missing_canonical_obligation">{t("financialWorkspace.reason.missingCanonical")}</option></select><p className="text-sm text-muted-foreground">{t("financialWorkspace.reason.guidance")}</p></div></>}
        {referenceAction && <>
